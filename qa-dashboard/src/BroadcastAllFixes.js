 ...
 *   ── atau satu per satu: ──
 *   2. fixDashboardLeadColumn()    → tambah kolom QA Team Lead di Config + Overview Dashboard
 ...
 *
 * PERUBAHAN:
 *   Dashboard Config  : tambah kolom "QA Team Lead" (F) antara PIC/Team dan Spreadsheet ID
...
function fixDashboardLeadColumn_() {
  ...
  var hasLead    = cfgHeaders.some(function(h){ return String(h).trim() === 'QA Team Lead'; });
  ...
  if (!hasLead) {
    ...
    // Style header
    cfg.getRange(3, 6).setValue('QA Team Lead')
    ...
    // Extend title/instruction merges using getMergedRanges (safe)
    try {
      var lastCfgCol = cfg.getLastColumn();
      [1, 2].forEach(function(titleRow) {
        var mrs = cfg.getRange(titleRow, 1, 1, lastCfgCol).getMergedRanges();
        mrs.forEach(function(mr) { mr.breakApart(); });
        cfg.getRange(titleRow, 1, 1, lastCfgCol).merge();
      });
    } catch(e) { Logger.log('Config merge (non-fatal): ' + e.message); }

    log.push('Config: kolom QA Team Lead ditambahkan (col F)');
  } else {
    log.push('Config: QA Team Lead sudah ada — skip');
  }
  ...
  if (ov) {
    ...
    if (!hasLeadOv) {
      ...
      // Fix group header MODULE INFO: extend merge to col 5
      // Use getMergedRanges to safely break only the affected merge
      try {
        var mergedRanges = ov.getRange(3, 1, 1, 10).getMergedRanges();
        mergedRanges.forEach(function(mr) {
          if (mr.getColumn() === 1 && mr.getRow() === 3) mr.breakApart();
        });
        ov.getRange(3, 1, 1, 5).merge().setValue('MODULE INFO')
          .setBackground('#263238').setFontColor('#FFFFFF')
          .setFontWeight('bold').setFontSize(9).setFontFamily('Arial')
          .setHorizontalAlignment('center').setVerticalAlignment('middle');
      } catch(e) { Logger.log('Overview group merge (non-fatal): ' + e.message); }

      // Extend title rows — use getMergedRanges for safety
      try {
        var lastOvCol = ov.getLastColumn();
        [1, 2].forEach(function(titleRow) {
          var mrs = ov.getRange(titleRow, 1, 1, lastOvCol).getMergedRanges();
          mrs.forEach(function(mr) { mr.breakApart(); });
          ov.getRange(titleRow, 1, 1, lastOvCol).merge();
        });
      } catch(e) { Logger.log('Overview title merge (non-fatal): ' + e.message); }
...
function fixApiExpectedResult_(spreadsheetId) {
  ...
  // Update title merge to cover new col count — safe approach
  try {
    var newCols = ws.getLastColumn();
    var mrs = ws.getRange(1, 1, 1, newCols).getMergedRanges();
    mrs.forEach(function(mr) { mr.breakApart(); });
    ws.getRange(1, 1, 1, newCols).merge();
  } catch(e) { Logger.log('API_Master title merge (non-fatal): ' + e.message); }
...
function buildAppendix_(ss) {
  ...
  apxRow_(ws, r, 'When to use?',
    'Use Scenario Outline + Examples when the SAME steps and\n' +
    'the SAME outcome type apply to multiple data variations.\n' +
    '\n' +
    'Key rules:\n' +
    '  1. All Examples rows = SAME type: all Positive OR all Negative.\n' +
    '     Never mix Positive and Negative in one Outline.\n' +
    '  2. Steps must be identical across all rows.\n' +
    '  3. Expected result structure must be the same for all rows.',
    null, 65); r++;

  apxRow_(ws, r, 'Format in Scenario column',
    'Write directly in the Scenario column (TC_Master / API_Master):\n\n' +
    'User Failed to Log In with <invalid_credential>\n' +
    'Examples:\n' +
    '- Invalid password\n' +
    '- Empty password\n' +
    '- Expired session\n\n' +
    'Each item = 1 TC executed separately.',
    null, 75); r++;

  apxRow_(ws, r, 'Example: NEGATIVE — Web',
    'Scenario Outline: User Failed to Log In with <invalid_credential>\n' +
    '  Given  user is on the Login page\n' +
    '  When   user submits login form with <invalid_credential>\n' +
    '  Then   login is rejected with error message <error_message>\n\n' +
    'Examples:\n' +
    '  | invalid_credential  | error_message              |\n' +
    '  | invalid password    | "Incorrect password"       |\n' +
    '  | empty password      | "Password is required"     |\n' +
    '  | expired session     | "Session has expired"      |\n\n' +
    '✓ All rows: same rejection outcome — Negative only.',
    '#FFEBEE', 90); r++;

  apxRow_(ws, r, 'Example: POSITIVE — Web',
    'Scenario Outline: Admin Successfully Creates User with Role <role>\n' +
    '  Given  admin is on the User Management page\n' +
    '  When   admin fills the form with valid data and assigns role <role>\n' +
    '  Then   user is created and appears in the user list\n\n' +
    'Examples:\n' +
    '  | role        |\n' +
    '  | Viewer      |\n' +
    '  | Operator    |\n' +
    '  | Supervisor  |\n\n' +
    '✓ All rows: same creation outcome — Positive only.',
    '#E8F5E9', 85); r++;

  apxRow_(ws, r, 'Example: NEGATIVE — API',
    'Scenario Outline: User Failed to Authenticate with <auth_condition> -- 401\n' +
    '  Given  user sends a request with <auth_condition>\n' +
    ...
    '  | empty token     |\n' +
    '  | expired token   |\n\n' +
    '✓ All rows: same 401 outcome — Negative only.\n' +
    '✗ Wrong: mixing 200 / 401 / 403 in one Outline.',
    '#FFEBEE', 95); r++;

  apxRow_(ws, r, 'Rules',
    '1. One Outline = one type only: all Positive OR all Negative.\n' +
    '   ✗ Wrong: mixing 200, 401, 403 in one Outline.\n' +
    '   ✓ Right: separate Outlines per status and outcome type.\n\n' +
    '2. All rows must share IDENTICAL steps and outcome structure.\n\n' +
    '3. Column Expected Result: write the common pattern.\n' +
    '   Row-level detail goes in the Examples column.\n\n' +
    '4. Max 5-6 items per Outline — split if more.',
    null, 80); r++;
  r++;
